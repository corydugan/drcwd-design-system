...
3b. **DEPLOY SOURCE — CONFIRMED 2026-06-18 (via Netlify MCP).** The live site (Netlify project `drcorydugan`, site id `41e94978-8512-456f-b29e-f955f935d64d`, team `69c469c724a4a9e8701de426`) was **manually uploaded** (deploy_source "api", "triggered by upload", commit_ref null) from `DR.CWD/netlify-deploy/` on 2026-06-12 — 6 files. It is **NOT connected to the GitHub repo** (`drcwd-website`, 8 files, stale at 2026-05-30). **Editing the GitHub repo ships nothing.** FIX (plan Phase 2): rebuild on v3 in the repo → connect Netlify→GitHub continuous deploy → retire the manual `netlify-deploy/` folder. Until Phase 2, `netlify-deploy/` IS the live source — do not archive it.
